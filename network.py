...
def build_point_pillar_graph(params: Parameters, batch_size: int = Parameters.batch_size):
    ...
    for n in range(4):
        ...
        x = tf.keras.layers.Conv2D(nb_channels, (3, 3), strides=S, padding="same", activation="linear",
                                   use_bias=False, name="cnn/block1/conv2d%i" % n)(x)
        x = tf.keras.layers.BatchNormalization(name="cnn/block1/bn%i" % n, fused=True)(x)
        x = tf.keras.layers.Activation("relu", name="cnn/block1/relu%i" % n)(x)
    x1 = x

    # Block2(2S, 6, 2C)
    for n in range(6):
        S = (2, 2) if n == 0 else (1, 1)
        x = tf.keras.layers.Conv2D(2 * nb_channels, (3, 3), strides=S, padding="same", activation="linear",
                                   use_bias=False, name="cnn/block2/conv2d%i" % n)(x)
        x = tf.keras.layers.BatchNormalization(name="cnn/block2/bn%i" % n, fused=True)(x)
        x = tf.keras.layers.Activation("relu", name="cnn/block2/relu%i" % n)(x)
    x2 = x
    ...
    for n in range(6):
        S = (2, 2) if n == 0 else (1, 1)
        x = tf.keras.layers.Conv2D(2 * nb_channels, (3, 3), strides=S, padding="same", activation="linear",
                                   use_bias=False, name="cnn/block3/conv2d%i" % n)(x)
        x = tf.keras.layers.BatchNormalization(name="cnn/block3/bn%i" % n, fused=True)(x)
        x = tf.keras.layers.Activation("relu", name="cnn/block3/relu%i" % n)(x)
    x3 = x
    ...
    clf = tf.keras.layers.Conv2D(nb_anchors * nb_classes, (1, 1), name="clf/conv2d", activation="softmax")(concat)
